...
class Joint(ContinualModel):
    ...
    def end_task(self, dataset):
        if dataset.SETTING != "domain-il":
            self.datasets.append(dataset.train_loader.dataset)
            self.current_task += 1
            ...
            temp_dataset = ConcatDataset(self.datasets)
            ...
            self.datasets.append(dataset.train_loader)
            # train
            if len(dataset.test_loaders) != dataset.N_TASKS:
                return
            ...
            for e in range(self.args.n_epochs):
                ...
                for i in range(int(math.ceil(len(all_inputs) / bs))):
                    ...
                    self.opt.step()
                    progress_bar(
                        i, int(math.ceil(len(all_inputs) / bs)), e, "J", loss.item()
                    )
