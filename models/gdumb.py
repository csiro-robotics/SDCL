...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument("--maxlr", type=float, default=5e-2, help="Penalty weight.")
    parser.add_argument("--minlr", type=float, default=5e-4, help="Penalty weight.")
    parser.add_argument(
        "--fitting_epochs", type=int, default=256, help="Penalty weight."
    )
    parser.add_argument(
        "--cutmix_alpha", type=float, default=None, help="Penalty weight."
    )
...
def fit_buffer(self, epochs):
    ...
    scheduler = lr_scheduler.CosineAnnealingWarmRestarts(
        optimizer, T_0=1, T_mult=2, eta_min=self.args.minlr
    )
    ...
    for epoch in range(epochs):
        ...
        while len(all_inputs):
            ...
            buf_inputs, buf_labels = (
                all_inputs[: self.args.batch_size],
                all_labels[: self.args.batch_size],
            )
            ...
            if self.args.cutmix_alpha is not None:
                inputs, labels_a, labels_b, lam = cutmix_data(
                    x=buf_inputs.cpu(), y=buf_labels.cpu(), alpha=self.args.cutmix_alpha
                )
