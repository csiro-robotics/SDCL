...
def get_parser() -> ArgumentParser:
    parser = ArgumentParser(
        description="Continual Learning via" " Meta-Experience Replay."
    )
    ...
    parser.add_argument(
        "--beta", type=float, required=True, help="Within-batch update beta parameter."
    )
    parser.add_argument(
        "--gamma",
        type=float,
    ...
    def draw_batches(self, inp, lab):
        ...
        for i in range(self.args.batch_num):
            if not self.buffer.is_empty():
                ...
                batches.append(
                    (inp.unsqueeze(0), torch.tensor([lab]).unsqueeze(0).to(self.device))
                )
        return batches
